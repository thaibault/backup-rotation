...
declare -g backupRotation_command='rsync --recursive --delete --perms --executability --owner --group --times --devices --specials --acls --links --super --whole-file --force --protect-args --hard-links --max-delete=1 --progress --human-readable --itemize-changes --verbose --exclude=.git --exclude=.npm --exclude=node_modules --exclude=.local "$source_path" "$target_file_path" && pushd "$(dirname "$target_file_path")" && tar --create --verbose --gzip --file "${target_file_path}${backupRotation_target_file_extension}" "$(basename "$target_file_path")"; popd && rm --recursive --verbose "$target_file_path"'
